...
impl PixelSink for PhysicalFramebuffer {
    fn write_pixel(&mut self, byte_offset: u64, bgra: u32) {
        // Only validated framebuffer offsets reach this sink. Entry rejects EL2 MMU/D-cache-on.
        // The Erista bootstack must have left the inspected scanout RAM writable and active.
        unsafe {
...
fn console() -> Console<PhysicalFramebuffer> {
    let layout = match FramebufferLayout::hekate_erista() {
        Ok(layout) => layout,
        Err(_) => park(),
    };
    Console::new(
...
fn launch_payload(
    payload: Payload,
    handoff: &[u64; 8],
    screen: &mut Console<PhysicalFramebuffer>,
) -> ! {
