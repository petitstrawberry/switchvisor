...
#[test]
fn usb_console_is_opt_in_and_preserves_the_original_register_contract() {
    let (mut payload, package) = fixture();
    payload.usb_uart = true;
    let bytes = payload.encode().unwrap();
    assert_eq!(u32::from_le_bytes(bytes[12..16].try_into().unwrap()), 3);
    assert_eq!(Payload::decode(&bytes), Ok(Some(payload)));
    assert_eq!(payload.source(&package).unwrap(), &package[0x4000..]);
    payload.preserve_boot_args = false;
    payload.registers[0] = 0x1234;
    assert_eq!(
...
#[test]
fn usb_control_is_independent_from_the_guest_console() {
